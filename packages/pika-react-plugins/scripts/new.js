...
function makeComponentOfSelf(name, type, mode) {
    const rootPath = componentRootPathDir.ui;
...
function changeComponentConfig({ name, chinese, type, mode }) {
    const lastDir = 'components';
    // index 修改
    {
...
getComponentFromArgs()
    .then(res => {
        if (res.name) {
            checkExistOrNot(res.name, res.type)
                .then(exist => {
                    if (exist) {
